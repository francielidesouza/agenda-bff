...
import readUserIdService from "../services/users/readUserId.service";
...
const readUserIdController = async (
  request: Request,
  response: Response
): Promise<Response> => {
  const userId: number = Number(request.params.id);

  const user: TUserResponse = await readUserIdService(userId);
